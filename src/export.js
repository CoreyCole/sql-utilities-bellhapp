...
getRestaurantLocation(RESTAURANT_NAME, )
 ...
 */
async function getRestaurantLocation(restaurantName, exportTime) {
...
/**
 * Exports the provided json in "pretty" format to the passed file
 * @param {object} restaurantLocation 
 * @param {string} exportFileName 
 * @param {string} exportFileDate 
 */
function exportJSON(restaurantLocation, exportFileName, exportFileDate) {
  restaurantLocation['_exportDate'] = exportFileDate
  shelljs.echo(prettyJSONStringify(restaurantLocation, {
    tab: '  ',
    spaceBeforeColon: ''
  })).toEnd(exportFileName)
  connection.end()
}
